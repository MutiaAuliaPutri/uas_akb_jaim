...
class MainActivity : AppCompatActivity(), onSelectData {
    ...
    private val textJSON: Unit
        get() {
            ...
            progressDialog.show()
            AndroidNetworking.get("http://api.aladhan.com/v1/timingsByCity?city=Bandung&country=Indonesia&method=5")
    ...
    private val dataJSON: Unit
        get() {
            ...
            progressDialog.setMessage("Mohon tunggu...")
            progressDialog.show()
            AndroidNetworking.get("http://api.aladhan.com/v1/hijriCalendarByCity?city=Jakarta&country=Indonesia&method=5&month=09&year=1445")
                    .setPriority(Priority.HIGH)
                    .build()
                    .getAsJSONObject(object : JSONObjectRequestListener {
                        override fun onResponse(response: JSONObject) {
